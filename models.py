...
class RNN(object):
	...
	def __init__(self, input_dim = 128, output_dim = 104 , learning_rate = 0.001):
		...
		self.gru_1 = Bidirectional(GRU(256, return_sequences=True, kernel_initializer='he_normal', name='gru1'),merge_mode="sum")(self.batch_norm)
		...
		self.y_pred = TimeDistributed(Dense(self.output_dim, kernel_initializer='he_normal',
		              name='dense2', activation='linear'))(self.gru_2)
		...
		self.loss_out = K.mean(warpctc_tensorflow.ctc(tf.transpose(self.y_pred, perm=[1,0,2]) , self.y_true, self.label_length, self.input_length))
		...
		self.optimizer = keras.optimizers.SGD(lr=learning_rate, decay=1e-6, momentum=0.9, nesterov=True, clipnorm = 200)
		self.update = self.optimizer.get_updates(self.model.trainable_weights, [], loss = self.loss_out)
		self.network_output = K.ctc_decode(Activation('softmax')(self.y_pred), self.input_length, True)[0][0]
		...
		self.predict_step = K.function([self.inp, self.input_length, K.learning_phase()], [self.network_output])
...
class CRNN(object):
	...
	def __init__(self, learning_rate = 0.001):
		...
		self.batch_norm = keras.layers.normalization.BatchNormalization()(self.inp)
		self.gru_1 = Bidirectional(GRU(rnn_size, return_sequences=True, kernel_initializer='he_normal', name='gru1'),merge_mode="sum")(self.batch_norm)
		self.gru_2 = Bidirectional(GRU(rnn_size, return_sequences=True, kernel_initializer='he_normal', name='gru2'),merge_mode="concat")(self.gru_1)
		self.y_pred = TimeDistributed(Dense(63, kernel_initializer='he_normal',
		              name='dense2', activation='linear'))(self.gru_2)
		...
		self.train_step = K.function([self.input_data, self.width, self.y_true, self.input_length, self.label_length, K.learning_phase()], \
							[self.loss_out, self.y_pred], updates = self.update)
		self.test = K.argmax(self.y_pred, axis=2)
		self.predict_step = K.function([self.input_data, self.width, self.input_length, K.learning_phase()], [self.network_output])
